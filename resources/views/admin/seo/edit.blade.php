...
@section('header', 'Edit SEO: ' . $seo->page_name)
    ...
    <div class="max-w-4xl mx-auto">
        <div class="mb-8 lg:mb-12">
            <a href="{{ route('admin.seo.index') }}"
                ...
                Back to List</a>
            <h1 class="text-2xl lg:text-3xl font-bold tracking-tight">SEO: {{ $seo->page_name }}</h1>
            <p class="text-[10px] lg:text-xs text-gray-500 uppercase tracking-widest mt-1">Configure meta tags for <span
                    class="text-black font-bold">{{ $seo->page_path }}</span></p>
        </div>
        ...
        <form action="{{ route('admin.seo.update', $seo->id) }}" method="POST" class="space-y-12 lg:space-y-20 pb-20">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-12">
                    <div class="space-y-2">
                        <label class="text-[9px] lg:text-[10px] uppercase tracking-widest font-bold text-gray-400">Page
                            Name</label>
                        <input type="text" name="page_name" value="{{ old('page_name', $seo->page_name) }}" required
                    ...
                    <div class="space-y-2">
                        <label class="text-[9px] lg:text-[10px] uppercase tracking-widest font-bold text-gray-400">Page Link
                            / Path</label>
                        <input type="text" name="page_path" value="{{ old('page_path', $seo->page_path) }}" required
                            class="w-full bg-transparent border-b border-black/10 focus:border-black py-4 text-sm outline-none transition-all"
                            placeholder="e.g. /services, /custom-page">
                    </div>
                </div>
            ...
            <section class="space-y-6 lg:space-y-8">
                ...
                <div class="space-y-6">
                    <div class="space-y-2">
                        ...
                        <p class="text-[10px] text-gray-400 mt-1">Recommended: 50-60 characters</p>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="meta_description" rows="3"
                            ...
                            placeholder="Detailed description of the page content">{{ old('meta_description', $seo->meta_description) }}</textarea>
                        <p class="text-[10px] text-gray-400 mt-1">Recommended: 150-160 characters</p>
                    </div>
            ...
            <section class="space-y-6 lg:space-y-8">
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="og_description" rows="2"
                        class="w-full bg-transparent border-b border-black/10 focus:border-black py-4 text-sm outline-none transition-all resize-none"
                        placeholder="Sharing Description">{{ old('og_description', $seo->og_description) }}</textarea>
                </div>
