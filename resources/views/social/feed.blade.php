...
<div class="min-h-screen pt-32 pb-24 relative z-10">
    <div class="max-w-[1400px] mx-auto px-6">
        ...
        @if(count($socialPosts) > 0)
        @php
            $groupedPosts = collect($socialPosts)->groupBy(function($post) {
                return strtolower($post['platform'] ?? 'updates');
            });

            // Normalize 'x' to 'twitter' for grouping purposes if it exists
            if ($groupedPosts->has('x')) {
                $xPosts = $groupedPosts->get('x');
                if ($groupedPosts->has('twitter')) {
                    $groupedPosts->put('twitter', $groupedPosts->get('twitter')->merge($xPosts));
                } else {
                    $groupedPosts->put('twitter', $xPosts);
                }
                $groupedPosts->forget('x');
            }

            // 1st is YouTube (already handled above)
            // 2nd Instagram, 3rd LinkedIn, 4th X
            $orderedPlatforms = ['instagram', 'linkedin', 'twitter'];
            
            $fallbackIcons = [
                'linkedin' => '<svg class="w-8 h-8 text-blue-600" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>',
                'twitter' => '<svg class="w-8 h-8 text-black" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>',
                'instagram' => '<svg class="w-8 h-8 text-pink-600" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.7-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>',
            ];
            $genericIcon = '<svg class="w-8 h-8 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9.5a2.5 2.5 0 00-2.5-2.5H15"></path></svg>';
        @endphp

        @foreach($orderedPlatforms as $platform)
            @if($groupedPosts->has($platform))
            @php 
                $posts = $groupedPosts->get($platform); 
                $displayPlatform = $platform === 'twitter' ? 'X' : ucfirst($platform);
            @endphp
            ...
            <div class="mb-24">
                <div class="flex flex-wrap items-center justify-between gap-4 mb-8" data-aos="fade-up">
                    ...
                    @php
                        $platformLink = isset($globalSocialLinks['social_media']) 
                            ? $globalSocialLinks['social_media']->firstWhere(function($item) use ($platform) {
                                $name = strtolower($item->platform_name);
                                if ($platform === 'twitter' && in_array($name, ['x', 'twitter'])) return true;
                                return $name === $platform;
                            }) 
                            : null;
                    @endphp
                    @if($platformLink)
                        <a href="{{ $platformLink->url }}" target="_blank" rel="noopener noreferrer" class="text-sm font-semibold uppercase tracking-widest hover:text-gray-600 transition-colors">Follow &rarr;</a>
                    @endif
                </div>
        ...
        @php
            // Catch any other platforms that weren't in our ordered list
            $otherPlatforms = $groupedPosts->except($orderedPlatforms);
        @endphp
